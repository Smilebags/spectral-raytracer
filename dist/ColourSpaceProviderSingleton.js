...
const sRGB = {
    ...
    to(colour) {
        ...
        return new Vec3(rec.x < 0 ? -1 : clamp(rec.x, 0, 1) ** (1 / 2.2), rec.y < 0 ? -1 : clamp(rec.y, 0, 1) ** (1 / 2.2), rec.z < 0 ? -1 : clamp(rec.z, 0, 1) ** (1 / 2.2));
    },
    from(colour) {
        const rec = new Vec3(colour.x ** 2.2, colour.y ** 2.2, colour.z ** 2.2);
