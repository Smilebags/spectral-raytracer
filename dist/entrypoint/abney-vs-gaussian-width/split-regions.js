...
function drawRing(highQuality) {
    ...
    const locusSamplePoints = locusPoints.map((point, index, arr) => {
        ...
        const colour = gaussianWideningStrategy.desaturate(point, state.desaturation, spectrumSampleCount, false).multiply(3);
    ...
    const magentaSamplePoints = magentaPoints.map((progress) => {
        const spectrumSamplePower = mapValue(state.desaturation ** 2, 0, 1, 6, 4) + SPECTRUM_SAMPLE_SHIFT;
        const spectrumSampleCount = Math.round(2 ** spectrumSamplePower);
        const redColour = gaussianWideningStrategy.desaturate(WAVELENGTH_HIGH, state.desaturation, spectrumSampleCount, false).multiply(3);
        const blueColour = gaussianWideningStrategy.desaturate(WAVELENGTH_LOW, state.desaturation, spectrumSampleCount, false).multiply(4);
        const radius = mapValue(state.desaturation, 0, 1, 1, 0);
        const radialProgress = mapValue(progress, 0, 1, Math.PI * 2 * LOCUS_LENGTH, Math.PI * 2);
        const location = new Vec2(Math.sin(radialProgress) * radius, Math.cos(radialProgress) * radius);
...
function createBoundaryValues(sampleCount) {
    return new Array(sampleCount)
        .fill(null)
        .map((item, index) => {
        const progress = index / (sampleCount - 1);
        const firstIndex = findFirstIndex(adjustedCumulative, (item) => item[0] >= progress);
...
//       );
// }
function createMagentaValues(sampleCount) {
    return new Array(sampleCount)
        .fill(null)
        .map((item, index) => {
