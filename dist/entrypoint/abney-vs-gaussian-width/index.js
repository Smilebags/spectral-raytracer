...
const curveCanvasOutput = new CanvasOutput(curveCanvasEl, SWATCH_WIDTH, 10, true, 'sRGB');
const abneyCanvasOutput = new CanvasOutput(abneyCanvasEl, SWATCH_WIDTH, 10, true, 'sRGB');
    ...
    for (let swatchIndex = 0; swatchIndex < STEP_COUNT; swatchIndex++) {
        ...
        for (let y = 0; y < SWATCH_SIZE; y++) {
            for (let x = 0; x < SWATCH_SIZE; x++) {
                canvasOutput.setPixel(colour.to(WORKING_SPACE).normalise().multiply(0.9999), new Vec2(x + (swatchIndex * SWATCH_SIZE), y));
            }
        }
    }
    canvasOutput.redraw();
...
function renderAbneySwatches(colour, canvasOutput) {
    ...
    for (let swatchIndex = 0; swatchIndex < STEP_COUNT; swatchIndex++) {
        ...
        for (let y = 0; y < SWATCH_SIZE; y++) {
            for (let x = 0; x < SWATCH_SIZE; x++) {
                canvasOutput.setPixel(lerpedColour.multiply(0.9999), new Vec2((swatchIndex * SWATCH_SIZE) + x, y));
            }
        }
    }
    canvasOutput.redraw();
